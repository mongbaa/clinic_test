        ...
        if (in_array('ct', $array_form_main_detail)) { //// นับครั้ง
            ...
            for ($x = 1; $x <= 4; $x++) {
                ...
                if (isset($_POST["time_start{$x}"])  && !empty($_POST["time_start{$x}"])) {
                    $time_start[$x] = $conn->real_escape_string($_POST["time_start{$x}"]);
                } else {
                    $time_start[$x] = date('H:i:s'); //'00:00:00';
                }

                if (isset($_POST["time_end{$x}"])  && !empty($_POST["time_end{$x}"])) {
                    $time_end[$x] = $conn->real_escape_string($_POST["time_end{$x}"]);
                } else {
                    $time_end[$x] = date('H:i:s'); // '00:00:00';
                }
